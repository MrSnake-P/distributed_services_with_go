...
import (
	...
	"distributed_services_with_go/internal/auth"
	"distributed_services_with_go/internal/config"
	"distributed_services_with_go/internal/log"
	"flag"
...
func setupTest(t *testing.T, fn func(*Config)) (rootClient, nobodyClient api.LogClient, cfg *Config, teardown func()) {
	...
	//require.NoError(t, err)
	//
	//dir, err := ioutil.TempDir("testData", "server_test")
	//require.NoError(t, err)
	//
	...
	if *debug {
		metricsLogFile, err := ioutil.TempFile("testdata", "metrics-*.log")
		require.NoError(t, err)
		t.Logf("metrics log file: %s", metricsLogFile.Name())

		traceLogFile, err := ioutil.TempFile("testdata", "traces-*.log")
		require.NoError(t, err)
...
func testProduceConsumeStream(t *testing.T, client, _ api.LogClient, config *Config) {
	...
	}}
	{
		...
		for offset, records := range records {
